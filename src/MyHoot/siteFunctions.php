		 ...
		 function __construct($questionNum)
		 {
			 ...
			 if ($result)
			 {
			    while($row = $result->fetch_assoc()){
			      ...
			      $long1 = $row["longg"];
			      $user_id = $row["user_id"];
						$this->allAnswers[]=Answer::addUser(new LatLong($lat1,$long1),$user_id,$this->correctAns);
...
class Answer
{
   ...
   var $totalMiles;
	   public static function loadCorrect($questionNum){
		 ...
		 public static function addUser($loc,$userID,$correct)
		 {
			  $answer=new self();
  ...
  if ($result)
  {
     ...
     $this->name= $row["name"];
      $this->totalMiles= $row["score"]+$this->distanceAway;
		 ...
		 {
			 //UPDATE `myhoot`.`answers` SET `answer` = 'fff' WHERE `answers`.`id` = 1;
//     $sql = "UPDATE `myhoot`.`answers` SET distanceAway='".$this->distanceAway."'WHERE game_id ='".$_SESSION["game_id"].
		//	                 "' AND questionNum='".$_SESSION["questionNumber"]."' AND user_id='".$this->user_id."'";
			 global $conn;
