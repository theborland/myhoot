...
class User {
  ...
  static sendPlaceComment(game_ID, allUsers, io) {
    ...
    for (var id in allUsers) {
      if (allUsers[id].answer != null) {
        var data = {
          roundPlace: totalUsers - allUsers[id].answer.roundPoints + 1,
          overallPlace: place,
          totalPlayers: totalUsers,
          color:allUsers[id].color
        };
        userInfo["n" + encodeURI(allUsers[id].name)] = data;
        place++;
      }
    }
  //  console.log(userInfo);
    if (place > 1)
      io.in(game_ID).emit('place', userInfo);
  }
  static compareDistanceAway(a, b) {
    if (a.answer != null && b.answer != null) {
      if (a.answer.distanceAway < b.answer.distanceAway) {
        ...
        return 1;
      } else return a.answer.submitOrder-b.answer.submitOrder;
  ...
  static loadAllUsers(game_id, callback) {
    ...
    db.getQuery("SELECT * from `users` WHERE `game_id`= '" + game_id + "'", function(err, results) {
      ...
      for (var i = 0; i < results.length; i++) {
        ...
        var name = "Weird name";
        try {
          name = decodeURI(results[i].name);
        } catch (e) {}
        allUsers[i] = new User(name, results[i].user_id);
  ...
  static createUser(game_id, name, session, req, callback) {
    session.name = name;
    // table="users";
    ...
    db.getQuery("SELECT * from `users` WHERE `game_id`= '" + game_id + "' AND `name`='" + name + "' LIMIT 1", function(err, results) {
      ...
      var query = "INSERT INTO `users` (`game_id`, `name`,`color`,`ip`) VALUES ('" + game_id + "','" + name + "','" + color + "','" + ip + "')";
      db.getQuery(query, function(err, results) {
        ...
        session.name = name;
        session.color = color;
        var query = "UPDATE games SET numOfUsers = numOfUsers+1 WHERE game_id = '"+game_id+"'";
        db.getQuery(query, function(err, results) {});
