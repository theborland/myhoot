...
class Question {
  ...
  constructor(type, session, callback) {
    // console.log("getting a qwuestion of type "+type);
    this.city = "";
    this.country = "";
    this.lat = "";;
    this.longg = "";;
    ...
    if (type != null) {
      ...
      //		this.getAge();
        ...
        this.getFlag(callback);
      else if (type == "custom")
        this.getCustom(callback,session);
      else if (type == "sports")
      {
        if (Math.random() < .3)
            this.getSports(callback);
        else
  ...
  getCustom(callback,session) {
    var  questionNum=session.questionNumber;

    var id=session.questionNumber;
    var qID=session.customQuestions[questionNum-1];
    console.log(session.customQuestions);
    var sql = "SELECT * FROM `data-custom` WHERE `id`="+(qID);
    if (isNaN(qID))
    {
      var geoID=qID.substring(3);
      sql = "SELECT * FROM `data-geo` WHERE `id`="+(geoID);
    }
    ...
    db.getQuery(sql, function(err, results) {
      if (err) {
        ...
        if (isNaN(qID)){
          ...
          alias.country = results[0].question;
          alias.type = "custom-"+results[0].type;
          alias.answer = results[0].answer;
          alias.lat = results[0].lat;
          alias.longg = results[0].longg;
          alias.min = results[0].min;
  ...
  getWeatherAvg(callback) {
    var regionsSelected = this.session.regionsSelected;

    var sql = "SELECT * FROM `data-geo" + "` WHERE `month1`> 0 AND ( ";
    for (var i = 0; i < regionsSelected.length; i++)
      sql += " `location` = '" + regionsSelected[i] + "' OR";
    sql = sql.substring(0, sql.length - 3);
    sql += "  )";
    ...
    this.type = "weather";
    var alias = this;
    console.log("IN HERE weatherAvg" + sql);
    db.getQuery(sql, function(err, results) {
      if (err || results.length == 0) {
        console.log(err);
        return alias.getPPT("places", callback);
      }
      alias.country = results[0].country;
      alias.city = results[0].city;
      alias.lat = results[0].lat;
      alias.longg = results[0].longg;
      var mon="month"+(new Date().getMonth()+1);

      alias.answer = results[0][mon];
        console.log("mon is "+alias.answer );
      alias.qID = results[0].id;
      console.log(alias.city);
      alias.country = results[0].country;
      var url = results[0].image;
  ...
  getAnimal(callback) {
    ...
    var sql = "SELECT * FROM `data-science-animals`";
    sql += " ORDER BY rand() LIMIT 1";
    this.type = "science-animals";
    var alias = this;
    console.log("IN HERE animals" + sql);
    db.getQuery(sql, function(err, results) {
      if (err || results.length == 0) {
        console.log(err);
        return alias.getQuestion("animal", callback);
      }
      alias.country = results[0].animal;
      alias.city = "";
      alias.answer = results[0].answer;
      //var url = results[0].flag;
      alias.qID = results[0].id;
      //console.log(alias.city);
      var url = results[0].image;
      alias.image = Question.getRandomUrl(url);
  ...
  getSports(callback) {
    ...
    var sql = "SELECT * FROM `data-sports-teams`";
    sql += " ORDER BY rand() LIMIT 1";
    this.type = "sports-teams";
    var alias = this;
    console.log("IN HERE SPORTS TEAMS" + sql);
    db.getQuery(sql, function(err, results) {
      if (err || results.length == 0) {
        console.log(err);
        return alias.getQuestion("sports", callback);
      }
      alias.country = results[0].park;
      alias.city = results[0].team;
      alias.lat = results[0].lat;
      alias.longg = results[0].longg;
      alias.answer = results[0].country;
      //var url = results[0].flag;
      alias.qID = results[0].id;
      console.log(alias.city);
      alias.image = "img/Logos/" + alias.city + ".svg";
      //alias.country = results[0].country;
      //console.log(alias);
      //console.log("question num"+this.qID)
      return callback(null, alias);
  ...
  getFlag(callback) {
    ...
    var sql = "SELECT * FROM `data-geo" + "` WHERE `flag`!='' AND ( ";
    for (var i = 0; i < regionsSelected.length; i++)
      sql += " `location` = '" + regionsSelected[i] + "' OR";
    sql = sql.substring(0, sql.length - 3);
    sql += "  )";
    ...
    this.type = "flag";
    var alias = this;
    console.log("IN HERE FLAG" + sql);
    db.getQuery(sql, function(err, results) {
      if (err || results.length == 0) {
        console.log(err);
        return alias.getPPT("people", callback);
      }
      alias.country = results[0].flag;
      alias.city = results[0].city;
      alias.lat = results[0].lat;
      alias.longg = results[0].longg;
      alias.answer = results[0].country;
      ...
      alias.qID = results[0].id;
      console.log(alias.city);
      alias.image = "img/flags/" + alias.country + ".png";
      alias.country = results[0].country;
      //console.log(alias);
      //console.log("question num"+this.qID)
      return callback(null, alias);
    ...
    if (Math.random() < .7) { //tmp .7
      ...
      db.getQuery(sql, function(err, results) {
        if (err) {
          ...
          alias.lat = 0;
          alias.longg = results[0].max;
          alias.extraInfo=results[0].worldAvg;
  ...
  getURL(file, vars, callback) {
    ...
    var url = 'https://nano.gameon.world/controller/estimation/' + file + '?' + stuff;
    var alias = this;
    http.get(url, (resp) => {
      return callback(null, alias);
    });
  ...
  getEstimation(callback) {
    ...
    if (random == 1) {
      ...
      this.image = '<div id="chart_div" style="width: 600px; height: 600px;    margin-left: auto;  margin-right: auto;"><img src="http://nano.gameon.world/controller/estimation/tmp/' + this.city + '.png"></div>';
      ...
      this.image = '<div id="chart_div" style="width: 1000px; height: 600px;    margin-left: auto;  margin-right: auto;"><img src="http://nano.gameon.world/controller/estimation/tmp/' + this.city + '.png"></div>';
  ...
  getMIH(callback) {
    // 		  //either get a cel age (75% of time )
    //var css = "width: 1000px; overflow-wrap: break-word; line-height: 100%;    margin-top: 40px;  font-size: 200;    text-align: center;    position: absolute;    padding: 20px;    left: 50%;    top: 50%;    transform: translateX(-50%) translateY(-50%);    var random = rand(1, 4);";
    var css = "";
    var random = rand(1, 8);
    //console.log("get ready" + random)
    ...
    if (random == 1) {
      ...
      var perc = rand(2, 14) * 5;
      var num1 = rand(40, 200);
      this.country = "SolvePOff";
      // this.city = rand(2001, 3000);
      this.answer = num1 - Math.round(num1 * perc / 100);
      this.min = 0;
      ...
      this.longg = this.max;
      this.qID = this.answer;
      var text = "$" + num1 + "<br>" + perc + "% off";
      this.city = text;
      this.image = '<div id="brain_div" style="font-size: 200; ' + css + '">' + text + '</div>';
      callback(null, this);

    } else if (random == 7) {
      var num1 = rand(40, 150);
      this.country = "SolveTip";
      // this.city = rand(2001, 3000);
      this.answer = num1 + Math.round(num1 * .2);
      this.min = 0;
      ...
      this.longg = this.max;
      this.qID = this.answer;
      var text = "$" + num1 + "<br>" + "Tip: 20%";
      this.city = text;
      this.image = '<div id="brain_div" style="font-size: 200; ' + css + '">' + text + '</div>';
      callback(null, this);

    } else if (random == 8) {
      var num2 = rand(40, 1500);
      var num1 = rand(3, num2 - 3);
      this.country = "SolvePerc";
      // this.city = rand(2001, 3000);
      this.answer = Math.round(num1 / num2 * 100);
      this.min = 0;
  ...
  getQuestionLabel() {
    if (this.type == "flag")
      return "Where is the capital";
    else if (this.type == "sports-teams")
      return "Where does this team play";
  ...
  getLabel() {
    ...
    if (this.country == "SolvePOff")
      return "Find the sale price - round to nearest $";
    if (this.country == "SolveTip")
      return "Find the total cost - round to nearest $";
    if (this.country == "SolvePerc")
      return "Estimate the percentage (round)";
    if (this.country.substring(0, 5) == "Solve")
      return "Solve";

    else if (this.type == "geo" || this.type == "weather" || this.type == "flag")
  ...
  getExtraInfo(){
    if (this.type=="facts")
         return " world avg is "+ this.extraInfo;
    else return "";
  }
  ...
  getQuestionTextEnd() {
    ...
    if (this.hasOwnProperty('country') && this.country.includes("?"))
    return "";
    else return "?";
  }
  ...
  getQuestionText() {
    ...
    if (this.type == "flag")
      return "For the country whose flag is shown:";
    if (this.type == "pop")
      return "What is the population of ";
    if (this.type == "weather")
      return "What is the average temp (day and night) this month in ";
    if (this.type == "age")
      return "How old is ";
    if (this.type == "science-animals")
      return "What is the maximal life expectancy of a(n) ";
    if (this.type == "time")
  ...
  static getUnitsAway(type, distanceAway) {
    ...
    else if (type == "age" || type == "time" || type=="science-animals")
      message = distanceAway + " years";
    else if (type == "geo" || type == "pt" || type == "places" || type=="flag")
  ...
  findBGColor() {
    ...
    if (this.type == "science" || this.type=="science-animals") return "842a7f";
  ...
  findUnit() {
    ...
    if (this.type == "science-animals") return " years";
    if (this.type == "age") return " years old";
    //  if (this.type == "age") return " %";
    console.log(this);
    if (this.wording.indexOf("%") != -1 || this.wording.indexOf("percent") != -1 || this.wording == "SolvePerc" || this.wording == "piePerc") return "%";
    if (this.wording == "SolvePOff" || this.wording == "SolvePTip") return "$";
    return " ";
  }
  findInputSize() {
  ...
  findRange() {
    ...
    if (this.type == "time") {
      ...
      else if (this.answer >= -200) {
        this.min = -200;
        this.max = 600;
        return " 'min': [-200],    '25%': [0],    '50%': [200],    '75%': [400],   'max': [600]";
      } else {
        this.min = -200;
        this.max = 600;
        return "   'min': [-800],    '25%': [-600],    '50%': [-400],    '75%': [-200],   'max': [0]";
      }
  ...
  addQuestion() {
    ...
    var sql;
    if (this.type == "geo" || this.type == "weather" || this.type == "pop" || this.type == "places" || this.type == "pt" || this.type == "flag" || this.type == "sports-teams" || this.type == "custom-geo")
    ...
    // console.log(sql);
    if (this.type == "custom-geo" || this.type == "custom-rand" )
          this.markCustomUsed();
    db.getQuery(sql, function(err, results) {
      if (err) {
        console.log(err);
  ...
  markCustomUsed() {
    ...
    var sql = "UPDATE `data-custom` SET `used`=`used`+1 WHERE `id`="+this.qID;
    ...
    db.getQuery(sql, function(err, results) {
      ...
      console.log("updated used +1 where question");
    });
  }
  ...
  loadImage() {
    ...
    if (this.type == "estimation") {
      ...
      if (this.country == "mapDist" || this.country == "percSq" || this.country == "ellipse") {
        return this.image = '<div id="chart_div" style="width: 600px; height: 600px;    margin-left: auto;' +
          '     		margin-right: auto;"><img src="http://nano.gameon.world/controller/estimation/tmp/' + this.city +
          '.png"></div>';
      }
    }
    if (this.type == "mih")
      return this.image = '<div id="brain_div" style="width: 600px; height: 600px;  color: rgba(236, 61, 7, 0.63); text-shadow: 5px 5px rgba(0,0,0,0.6);   font-size: 120;   margin-left: auto;  margin-right: auto;">' + this.city + '</div>';
    ...
    // return this.getSimulationFile('piePercent', vars, callback);
    // }
    if (this.type == "pop" || this.type == "weather" || this.type == "flag" || this.type == "sports-teams")
    ...
    else if (this.type == "custom-geo" || this.type == "custom-rand") {
        sql = "SELECT * FROM `data-custom` WHERE `id`='" + this.qID + "' LIMIT 1"; //" WHERE `id`='3'";
    } else
  ...
  static loadQuestion(game_id, callback) {
    ...
    db.getQuery(sql, function(err, results) {
      ...
      if (type == "geo" || type == "pop" || type == "weather" || type == "pt" || type == "estimation" || type == "mih" || type == "flag" || type == "sports-teams") {
