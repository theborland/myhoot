...
class GameLogic {
  static setupGame(post, session, callback) {
    ...
    if (post.gsTemp == "true") gamesSelected.push("weather");
      if (post.gsFlags == "true") gamesSelected.push("flags");
    ...
    if (post.gsGeo == "true") {
      var numGeos=1;
      if (gamesSelected.length>0 && gamesSelected.length<4)
          numGeos=gamesSelected.length;
      else if (gamesSelected.length>=4)
          numGeos=3;
    ...
    if (post.gsCustom=="true"){
      gamesSelected.push("custom");
      post.numRounds=10;
      session.customID=post.customID;
    }
    ...
    if (post.gsCustom=="true")
    {
          db.getQuery("SELECT * FROM `customQuizzes` WHERE id ='" + session.customID + "'", function(err, results) {
            if (err)
              return callback(true);
            else {
              var q=results[0].sequence.split(",");
              session.customQuestions=q;
              session.numRounds=q.length;
