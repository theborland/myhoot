...
class Answer {
  ...
  static findPlace(distanceAway, question_id, callback) {
    //now find place
    var sql = "select question_id, count(*) total, sum(case when distanceAway >= '" + distanceAway + "' then 1 else 0 end) worse from `answersShort` WHERE question_id='" + question_id + "' AND distanceAway>=0";
    db.getQuery(sql, function(err, results) {
      if (err) {
        console.log(err);
        return callback(true);
