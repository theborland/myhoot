...
class DB {
  static getNumAnswers(callback) {
    if (oldAnswerCount==0 || oldAnswerCount==null){
        console.log("trying to update oldAnswerCount");
        this.cleanAnswers(function(){});
      }
    DB.getQuery("SELECT COUNT(id) as total FROM answers", function(err, results) {
      ...
      results = "" + ((results[0].total) + oldAnswerCount);
      callback(false, results);
    });
  }

  static cleanAnswers(callback) {
    var SQL="INSERT INTO answersShort (`question_id`,`distanceAway`,`submitTime`) SELECT `question_id`,`distanceAway`,`submitTime` FROM answers";
    DB.getQuery(SQL, function(err, results) {
      ...
      var SQL2="DELETE from answers";
      DB.getQuery(SQL2, function(err, results) {
        if (err) {
          console.log("error with move/delete" + err);
          return "";
        }
        DB.getQuery("SELECT COUNT(id) as total FROM answersShort", function(err, results) {
          if (err) {
            console.log("error with move/delete" + err);
            return "";
          }
          else
          oldAnswerCount=results[0].total;
          callback(false);
        });
      });
    }
